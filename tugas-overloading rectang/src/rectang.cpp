...
rectang::rectang(float midpoint_x, float midpoint_y, float length, float width){
...
bool rectang::operator==(rectang const &new_var)const{
...
rectang rectang::operator+(rectang const &new_var){
    rectang temp(0,0,0,0);
    if (*this == new_var){
...
float rectang::operator[](int pilihan){
    switch (pilihan){
    case 1:
        return this->x_min;
        break;
